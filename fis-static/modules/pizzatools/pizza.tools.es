         ...
         layer.loading(1);
            ...
            if (method == "get" || method == "del") {
                return new Promise(function(resolve, reject) {
                    superagent[method].call(this, url).query(data).accept('application/json').end(function(err, res) {
                        if (err || !res.ok) {
                            reject(err || res.ok);
                        }
                ...
                return new Promise(function(resolve, reject) {
                    superagent[method].call(this, url).send(data).accept('application/json').end(function(err, res) {
                        if (err || !res.ok) {
                            reject(err || res.ok);
                        }
                        resolve(JSON.parse(res.body));
